...
def process_data(data):
    filtered_data = band_pass_filter(data, 0.5, 50)
    f, Pxx = calculate_psd(filtered_data)
...
class EEGReceiver:
    ...
    def process_data(self):
        rf_model = load('../model_P_CCA/best_rf_classifier.joblib')
        svm_model = load('../model_P_CCA/best_svm_classifier.joblib')
        lda_model = load('../model_P_CCA/best_lda_classifier.joblib')
        knn_model = load('../model_P_CCA/best_knn_classifier.joblib')
        ...
        cca_model_ml = load('../model_P_CCA/cca_model_ml.joblib')
        cca_model_dl = load('../model_P_CCA/cca_model_dl.joblib')
        while True:
            if self.ready_for_processing and not self.data_queue.empty():
                ...
                data_fft_oz = process_data(data_oz)
                data_fft_o1 = process_data(data_o1)
                data_fft_o2 = process_data(data_o2)
                combined = np.hstack((data_fft_oz, data_fft_o1, data_fft_o2))
                print(combined.shape)
                # Transform data using loaded CCA model
                combined_data_cca = cca_model_ml.transform(combined.reshape(1, -1))
                # Features Normalization
                scaler = StandardScaler()
                combined_test_use = scaler.fit_transform(combined_data_cca)

                # Combine with the original data
                combined = np.hstack((combined_test_use, combined_test_use))
                ...
                print(combined.shape)
                # Transform data using loaded CCA model
                combined_data_cca = cca_model_dl.transform(combined.reshape(1, -1))
                # Features Normalization
                scaler = StandardScaler()
                combined_test_use = scaler.fit_transform(combined_data_cca)
                ...
                combined_test_scaled_cnn = scaler.fit_transform(combined)
                combined_test_scaled_lstm = scaler.fit_transform(combined)
